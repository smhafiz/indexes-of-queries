...
{
	std::ofstream myfile;
	myfile.open ("testDataStatNOPsG.txt");
	srand (time(NULL));
	...
	for (long p = 2; p <= r; p <<= 1)
	{
		for (long u = 1; u <= max_u; u <<=1)
		{
			...
			while(timeSpend < nsInOneSec)
			{
				...
				cudaMemcpy(queryVector_d+i, queryVector+i, p*sizeof(unsigned), cudaMemcpyHostToDevice);
				dvsmMultKernel<<<NDVSM,THREADSPERBLOCKDVSM>>>(lengthOfResultVectorReduced,columnPtr_d, rowIndex_d, valueArray_d, queryVector_d+i, resultSparseVectorValueArray_d+i);
				cudaMemcpy(resultSparseVectorValueArrayDD+i, resultSparseVectorValueArray_d+i, lengthOfResultVectorReduced*sizeof(unsigned long long int), cudaMemcpyDeviceToHost);
