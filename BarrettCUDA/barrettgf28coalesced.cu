...
/*__device__ __forceinline__ uint8_t multiply_GF216_Element(const uint8_t x, const uint8_t y,  const uint8_t * d_GF216_log_table, const uint8_t * d_GF216_exp_table)
{
    if(x == 0 || y == 0) return 0;
...
__device__ __forceinline__ uint8_t multiply_GF28_Element(const uint8_t x, const uint8_t y,  const uint8_t * d_GF28_mult_table)
{
    if(x == 0 || y == 0) return 0;
    return d_GF28_mult_table[x*256+y];
}


void SpMV_gf28_cpu(int u_wrap, uint8_t * response, const uint8_t * query, const SparseMatrix<uint> & matrix)
{
	for (int i = 0; i < matrix.ncols; i++)
	{
		...
		//if(i==101) printf("CPU: val: %u, col: %u, rows: %u, query: %u, temp: %u\n",(val      )& 0xFFFF,col,matrix.l_rows[col],query[matrix.l_rows[col]],multiply_GF2E<uint8_t>(((val      )& 0xFFFF), query[matrix.l_rows[col]]));
		temp ^= multiply_GF2E<uint8_t>(((val      )& 0xFF), query[matrix.l_rows[col++]]);
		//if(i==101) printf("CPU: val: %u, col: %u, rows: %u, query: %u, temp: %u\n",(val >> 16)& 0xFFFF,col,matrix.l_rows[col],query[matrix.l_rows[col]],multiply_GF2E<uint8_t>(((val  >> 16)& 0xFFFF), query[matrix.l_rows[col]]));	
		temp ^= multiply_GF2E<uint8_t>(((val >> 8)& 0xFF), query[matrix.l_rows[col++]]);
		temp ^= multiply_GF2E<uint8_t>(((val >> 16)& 0xFF), query[matrix.l_rows[col++]]);
		temp ^= multiply_GF2E<uint8_t>(((val >> 24)& 0xFF), query[matrix.l_rows[col++]]);
			
	    }
	    response[i] = temp;
	if(i == 1087)		printf("CPU response[%d]: %u\n",i,response[i]);
	}
}
	...
	const uint * vals, const uint ncols, const uint * cols, const uint * rows,  const uint8_t * d_GF28_mult_table)
{
    ...
    register uint8_t temp = 0;
		...
		temp ^= multiply_GF28_Element(((val      )& 0xFF), query[rows[col++]], d_GF28_mult_table);
		...
		temp ^= multiply_GF28_Element(((val >> 8)& 0xFF), query[rows[col++]],d_GF28_mult_table);
		temp ^= multiply_GF28_Element(((val >> 16)& 0xFF), query[rows[col++]],d_GF28_mult_table);
		temp ^= multiply_GF28_Element(((val >> 24)& 0xFF), query[rows[col++]],d_GF28_mult_table);
	}
    response[i] = temp;
//	if(i == 1087) printf("GPU response[%d]: %u\n",i,response[i]);
}
...
{
    cudaMemcpyAsync(d_query, l_query, matrix.nrows*sizeof(uint8_t), cudaMemcpyHostToDevice, stream);
Check_CUDA_Error("GF28: cudaMemcpyAsync:cudaMemcpyHostToDevice");
    const dim3 Dg(NUM_BLOCKS(matrix.ncols), 1, 1);
    const dim3 Db(THREADS_PER_BLOCK(matrix.ncols), 1, 1);
    const size_t Ns = 0;

    SpMV_kernel<<< Dg, Db, Ns, stream >>> (u_wrap, d_response, d_query, matrix.nvals, matrix.d_vals, matrix.ncols, matrix.d_cols, matrix.d_rows, d_GF28_mult_table);
Check_CUDA_Error("GF28:SpMV_kernel");
    cudaMemcpyAsync(l_response, d_response, matrix.ncols*sizeof(uint8_t), cudaMemcpyDeviceToHost, stream);
Check_CUDA_Error("GF28:cudaMemcpyAsync:cudaMemcpyDeviceToHost");
}
template <typename T>
	...
	struct SparseMatrix<T> & matrix)
{
    ...
    if (!valstream) { cerr << "Error: opening VALS files\n"; exit(-1); }
    	...
    	NTL::ZZ temp_zz;
	valstream >> temp_zz;
	modulus = NTL::trunc_ZZ(temp_zz, 8);
...
//std::cout << "matrix.nvals:\t" << matrix.nvals << "\n";
	...
	{	int number_of_nnz_in_a_col = matrix.l_cols[i+1] - matrix.l_cols[i];
		...
		{	c++;
			...
			NTL::ZZ tmp_zz;
			valstream >> tmp_zz;
			uint tmp = NTL::trunc_long(tmp_zz, 8);
...
int main(int argc, char ** argv)
{
    ...
    uint8_t * d_GF28_mult_table;//65536

    cudaMalloc((void**)&d_GF28_mult_table, sizeof(GF28_mult_table));
    cudaMemcpy(d_GF28_mult_table, &GF28_mult_table, sizeof(GF28_mult_table),cudaMemcpyHostToDevice);
    ...
    uint8_t * cpu_l_response = (uint8_t*) malloc(nstreams*matrix.ncols * sizeof(uint8_t));
	...
	for (int i = 0; i < nstreams; i++)
	{
	    ...
	    //SpMV_gf28_cpu(u_wrap, cpu_l_response, __l_query, matrix);
