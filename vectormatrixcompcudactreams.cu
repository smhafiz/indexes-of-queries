...
{
	std::ofstream myfile;
	myfile.open ("testDataStatNOPsF.txt");
	srand (time(NULL));
	...
	for (long p = 2; p <= r; p <<= 1)
	{
		for (long u = 1; u <= max_u; u <<=1)
		{
			...
			while(timeSpend < nsInOneSec)
			{
				...
				cudaMemcpyAsync(queryVector_d+i, queryVector+i, p*sizeof(unsigned), cudaMemcpyHostToDevice, streams[i]);
				dvsmMultKernel<<<NDVSM,THREADSPERBLOCKDVSM,0,streams[i]>>>(lengthOfResultVectorReduced,columnPtr_d, rowIndex_d, valueArray_d, queryVector_d+i, resultSparseVectorValueArray_d+i);
				cudaMemcpyAsync(resultSparseVectorValueArrayDD+i, resultSparseVectorValueArray_d+i, lengthOfResultVectorReduced*sizeof(unsigned long long int), cudaMemcpyDeviceToHost, streams[i]);
			...
			std::cout<< "In 1 second GPU handles "<< numberOfOps <<" operations\n";
			myfile<< "In 1 second GPU handles "<< numberOfOps <<" operations\n";
