...
__device__ __forceinline__ void normalize(const uint96 & aLo, const uint96 & aHi, const uint2X<uint96> & s)
{
    uint * _aLo = (uint *)&aLo;
    uint * _aHi = (uint *)&aHi;
	...
	"subc.cc.u32	 %4, %4,%10;\n\t"	// r4-=(r10+c)
	"subc.u32	 %5, %5,%11;\n\t"	// r5-=(r11+c)
	: "+r"(_aLo[0]), "+r"(_aLo[1]), "+r"(_aLo[2]), "+r"(_aHi[0]), "+r"(_aHi[1]), "+r"(_aHi[2])
	: "r"(_s[0]), "r"(_s[1]), "r"(_s[2]), "r"(_s[3]), "r"(_s[4]), "r"(_s[5]));
}

__device__ __forceinline__ void sub(const uint96 & aLo, const uint & aHi, const uintXp<uint96> & r)
{
    uint * _aLo = (uint *)&aLo;
    uint * _aHi = (uint *)&aHi;
	...
	"subc.cc.u32	 %2, %2, %6;\n\t"	// r2-=( r5+c)
	"subc.u32	 %3, %3, %7;\n\t"	// r3-=( r5+c)
	: "+r"(_aLo[0]), "+r"(_aLo[1]), "+r"(_aLo[2]), "+r"(_aHi[0])
	: "r"(_r[0]), "r"(_r[1]), "r"(_r[2]), "r"(_r[3]));
}

__device__ __forceinline__ uintXp<uint96> get_q(const uint & aLo, const uint96 & aHi, const uint96 & mu)
{
	...
	: "r"(_aLo[0]), "r"(_aHi[0]), "r"(_aHi[1]), "r"(_aHi[2]), "r"(_mu[0]), "r"(_mu[1]), "r"(_mu[2]));
	printf("Inside get q\nmu:\t\t\t%u\t%u\t%u\n",_mu[0],_mu[1],_mu[2]);
	printf("a:\t\t\t%u\t%u\t%u\t%u\n",_aLo[0],_aHi[0],_aHi[1],_aHi[2]);
...
__device__ __forceinline__ void mad(const uint96 & aLo, const uint96 & aHi, const uint & overflow, const uint96 & b, const uint96 & c)
{
    uint * _aLo = (uint *)&aLo;
    uint * _aHi = (uint *)&aHi;
    const uint * _b = (uint *)&b;
    const uint * _c = (uint *)&c;
    printf("\nBefore mad\na:\t\t\t%u\t%u\t%u\t%u\t%u\t%u\n",_aLo[0],_aLo[1],_aLo[2],_aHi[0],_aHi[1],_aHi[2]);
	...
	"addc.cc.u32	 %5, %5,  0    ;\n\t"	// r5+=c
	"addc.u32	 %6, %6,  0    ;\n\t"	// r6+=c
	: "+r"(_aLo[0]), "+r"(_aLo[1]), "+r"(_aLo[2]), "+r"(_aHi[0]), "+r"(_aHi[1]), "+r"(_aHi[2]), "+r"(overflow)
