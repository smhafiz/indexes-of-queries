...
void initBarret(const NTL::ZZ & modulus_zz, struct BarretParams<T> & barret)
{
	...
	cout<< "From InitBarret mu:\t" << conv<uint>(to_ZZ(mu)) <<"\t"<< conv<uint>(to_ZZ(mu) >> 32)<<"\t"<< conv<uint>(to_ZZ(mu) >> 64) <<"\t"<< conv<uint>(to_ZZ(mu) >> 96) <<"\n";
...
__global__ void smm_kernel(T * result, const T * query, const uint * rows, const uint * cols, const uintX * vals, const T * modulus, const T * mu, const T * subtrahends)
{
	...
	printf("Before normalize a:\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\n\n",result[i].x, result[i].y, result[i].z, result[i].w, a.x, a.y, a.z, a.w);
	//normalize(result[i], a, subtrahends + 2*overflow);
	printf("After normalize a:\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\n\n",result[i].x, result[i].y, result[i].z, result[i].w, a.x, a.y, a.z, a.w);
	uintXp<T> q = get_q(result[i], a, *mu);
	printf("q:\t\t\t%u\t%u\t%u\t%u\t%u\n\n",q.lo.x,q.lo.y,q.lo.z,q.lo.w,q.hi);
	uintXp<T> r2 = get_r2(q, *modulus);
	printf("r2:\t\t\t%u\t%u\t%u\t%u\n\n",r2.lo.x,r2.lo.y,r2.lo.z,r2.lo.w);
	sub(result[i], a, r2);
	printf("a after sub r2:\t\t%u\t%u\t%u\t%u\n\n",result[i].x, result[i].y, result[i].z,result[i].w);
}
...
{
	NTL::ZZ modulus_zz = NTL::RandomPrime_ZZ(128);
	NTL::ZZ_p::init(modulus_zz);
	const NTL::ZZ mu_zz = NTL::power2_ZZ(2 *  128) / modulus_zz;
	uintX modulus = {0};
	...
	cout << "\nb:\t\t\t" << (b) << "\nc:\t\t\t" << (c)  << "\nmodulus:\t\t" << (modulus_zz) << "\n\t\t\t" << conv<uint>(modulus_zz) << "\t" << conv<uint>(modulus_zz>>32) << "\t" << conv<uint>(modulus_zz>>64) << "\t" << conv<uint>(modulus_zz>>96) << "\n";
	...
	ZZ a_zz = b_zz *c_zz;
	cout << "b_zz:\t\t\t" << conv<uint>(b_zz) << "\t" << conv<uint>(b_zz>>32) << "\t" << conv<uint>(b_zz>>64) << "\t" << conv<uint>(b_zz>>96) << "\n";
	cout << "c_zz:\t\t\t" << conv<uint>(c_zz) << "\t" << conv<uint>(c_zz>>32) << "\t" << conv<uint>(c_zz>>64) << "\t" << conv<uint>(c_zz>>96) << "\n";
	cout<< "a_zz:\t\t\t" << a_zz << "\n\t\t\t" << conv<uint>(a_zz) << "\t" << conv<uint>(a_zz>>32) << "\t" << conv<uint>(a_zz>>64) << "\t" << conv<uint>(a_zz>>96)<< "\t" << conv<uint>(a_zz>>128) << "\t" << conv<uint>(a_zz>>160) << "\t" << conv<uint>(a_zz>>192) << "\t" << conv<uint>(a_zz>>224) << endl;
	//Barret start
	...
	cout << "q:\t\t\t" << conv<uint>(q3) << "\t" << conv<uint>(q3>>32) << "\t" << conv<uint>(q3>>64) << "\t" << conv<uint>(q3>>96) << "\n";
	...
	cout << "r:\t\t\t" << conv<uint>(r2) << "\t" << conv<uint>(r2>>32) << "\t" << conv<uint>(r2>>64) << "\t" << conv<uint>(r2>>96) << "\n";
	...
	cout << "Barrett result:\t\t" << r << "\n\t\t\t" << conv<uint>(r) << "\t" << conv<uint>(r>>32) << "\t" << conv<uint>(r>>64) << "\t" << conv<uint>(r>>96) << "\n";

	//Barrett finish
	ZZ res;
	NTL::rem(res, a_zz, modulus_zz);
	cout << "ZZ result:\t\t" << res << "\n\t\t\t" << conv<uint>(res) << "\t" << conv<uint>(res>>32) << "\t" << conv<uint>(res>>64) << "\t" << conv<uint>(res>>96) << "\n";
	...
	cout << "Response:\t\t" << conv<uint>(to_ZZ(*l_response)) << "\t" <<conv<uint>(to_ZZ(*l_response)>>32) << "\t" <<conv<uint>(to_ZZ(*l_response)>>64) << "\t" <<conv<uint>(to_ZZ(*l_response)>>96) <<endl;
	delete [] streams;
