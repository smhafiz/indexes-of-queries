...
{
	const NTL::ZZ mu_zz = NTL::power2_ZZ(2 * 96) / modulus_zz;
	const T modulus = to_uint<T>(modulus_zz);
	const T mu = to_uint<T>(mu_zz); // loses high order bit!
	
	cout<< "From InitBarret mu:\t" << conv<uint>(to_ZZ(mu)) <<" "<< conv<uint>(to_ZZ(mu) >> 32)<<" "<< conv<uint>(to_ZZ(mu) >> 64)<<"\n";
	...
	for (int i = 0; i < barret.u; ++i)
	{
		...
		subtrahends[i].hi = to_uint<T>(subtrahend >> BITS_PER_LIMB * LIMBS_IN(T));
	}
...
template <typename T>
__global__ void smm_kernel(T * result, const T * query, const uint * rows, const uint * cols, const uintX * vals, const T * modulus, const T * mu, const uint2X<T> * subtrahends)
{
	...
	{
		mad(a, overflow, vals[j], query[rows[j]]);
	}
	printf("Before normalize a:\t%u\t%u\t%u\t%u\t%u\t%u\n\n",a.lo.x, a.lo.y, a.lo.z, a.hi.x, a.hi.y,a.hi.z);
	normalize(a, subtrahends[overflow]);
	printf("After normalize a:\t%u\t%u\t%u\t%u\t%u\t%u\n\n",a.lo.x, a.lo.y, a.lo.z, a.hi.x, a.hi.y,a.hi.z);
	uintXp<T> q = get_q(a, *mu);
	printf("q:\t\t\t%u\t%u\t%u\t%u\n\n",q.lo.x,q.lo.y,q.lo.z,q.hi);
	uintXp<T> r2 = get_r2(q, *modulus);
	printf("r2:\t\t\t%u\t%u\t%u\n\n",r2.lo.x,r2.lo.y,r2.lo.z);
	sub(a, r2);
	printf("a after sub r2:\t\t%u\t%u\t%u\n\n",a.lo.x,a.lo.y,a.lo.z);
	//printf("result[i] before:\t%u\t%u\t%u\n\n",result[i].x,result[i].y,result[i].z);
	result[i] = a.lo;
	//printf("result[i] after:\t%u\t%u\t%u\n\n",result[i].x,result[i].y,result[i].z);
}

template <typename T>
void smm(T * l_response, T * l_query, T * d_response, T * d_query, const cudaStream_t & stream, const uint nrows, const uint * rows, const uint ncols, const uint * cols, const uint nvals, const uintX * vals, const T * modulus, const T * mu, const uint2X<T> * subtrahends)
{
...
{
	NTL::ZZ modulus_zz = NTL::RandomPrime_ZZ(96);
	NTL::ZZ_p::init(modulus_zz);
	const NTL::ZZ mu_zz = NTL::power2_ZZ(2 *  96) / modulus_zz;
	const uintX modulus = to_uint<uintX>(modulus_zz);
	const uintX mu = to_uint<uintX>(mu_zz);
	int nthreads = 1;
	...
	cout << "\nb:\t\t\t" << (b) << "\nc:\t\t\t" << (c)  << "\nmodulus:\t\t" << (modulus_zz) << "\n\t\t\t" << conv<uint>(modulus_zz) << "\t" << conv<uint>(modulus_zz>>32) << "\t" << conv<uint>(modulus_zz>>64) << "\n";
	/*cout << "barret.mu:\t" << to_ZZ(barret.mu->x); << "\t" << conv<uint>(to_ZZ(barret.mu->y)) << "\t" << conv<uint>(to_ZZ(barret.mu->z)) << endl;
	cout << "barret.modulus:\t" << conv<uint>(to_ZZ(barret.modulus->x)) << "\t" << conv<uint>(to_ZZ(barret.modulus->y)) << "\t" << conv<uint>(to_ZZ(barret.modulus->z)) << endl;*/
	

	//ZZ test start
	const uint16_t limbs = 3;
	...
	ZZ a_zz = b_zz *c_zz;
	cout << "b_zz:\t\t\t" << conv<uint>(b_zz) << "\t" << conv<uint>(b_zz>>32) << "\t" << conv<uint>(b_zz>>64) << "\n";
	cout << "c_zz:\t\t\t" << conv<uint>(c_zz) << "\t" << conv<uint>(c_zz>>32) << "\t" << conv<uint>(c_zz>>64) << "\n";
	cout<< "a_zz:\t\t\t" << a_zz << "\n\t\t\t" << conv<uint>(a_zz) << "\t" << conv<uint>(a_zz>>32) << "\t" << conv<uint>(a_zz>>64) << "\t" << conv<uint>(a_zz>>96)<< "\t" << conv<uint>(a_zz>>128) << "\t" << conv<uint>(a_zz>>160) << endl;
	//Barret start
	...
	cout << "q:\t\t\t" << conv<uint>(q3) << "\t" << conv<uint>(q3>>32) << "\t" << conv<uint>(q3>>64) << "\n";
	...
	cout << "r:\t\t\t" << conv<uint>(r2) << "\t" << conv<uint>(r2>>32) << "\t" << conv<uint>(r2>>64) << "\n";
	...
	cout << "Barrett result:\t\t" << r << "\n\t\t\t" << conv<uint>(r) << "\t" << conv<uint>(r>>32) << "\t" << conv<uint>(r>>64) << "\n";

	//Barrett finish
	ZZ res;
	NTL::rem(res, a_zz, modulus_zz);
	cout << "ZZ result:\t\t" << res << "\n\t\t\t" << conv<uint>(res) << "\t" << conv<uint>(res>>32) << "\t" << conv<uint>(res>>64) << "\n";
	...
	cout << "Response:\t\t" << conv<uint>(to_ZZ(*l_response)) << "\t" <<conv<uint>(to_ZZ(*l_response)>>32) << "\t" <<conv<uint>(to_ZZ(*l_response)>>64) <<endl;
	delete [] streams;
